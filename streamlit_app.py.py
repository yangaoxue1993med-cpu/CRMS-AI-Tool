...
# V1.9（按用户需求：新增饼图配色方案选择 + Timeline标签精简 + 研究准备阶段新增项目立项）
# =====================================================
...
BUDGET_ALLOCATION_RULES = {
    ...
    "领头中心费用": {"phases": [{"level1": "研究中心启动", "level2": "领头中心启动", "weight": 1.0, "mode": "lump_start"}]},
...
with st.sidebar.expander("预算模块1：研究中心花费", expanded=True):
    ...
    if st.checkbox("领头中心费用", value=True):
        amount = st.number_input("领头中心费用 金额", min_value=0.0, step=10000.0, value=200000.0, key="site_lead_a")
        site_total_rmb += add_cost_item(rows, "预算模块1：研究中心花费", "领头中心费用", amount, 1, "", input_currency, fx_usd_to_rmb, show_both_currency)
...
with tab_summary:
    ...
    summary_html = "<table style='width:100%; border-collapse:collapse; font-size:18px;'>"
    for i, row in df_summary.iterrows():
        ...
        summary_html += f"""
        <tr>
            <td style='border:1px solid #222; padding:10px; width:48%; font-weight:700;'>{label}</td>
            <td style='border:1px solid #222; padding:10px; width:52%;'>{value_html}</td>
        </tr>
        """
    summary_html += "</table>"
